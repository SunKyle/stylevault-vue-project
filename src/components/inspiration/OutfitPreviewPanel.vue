<template>
  <div class="bg-gradient-to-br from-indigo-50 via-white to-purple-50 rounded-2xl shadow-lg overflow-hidden border border-indigo-100 h-full flex flex-col transform transition-all duration-300 hover:shadow-xl hover:shadow-indigo-100">
    <div class="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 p-5 border-b border-indigo-100 backdrop-blur-sm">
      <div class="flex justify-between items-center">
        ...
        <div class="flex items-center gap-2">
          <span class="bg-white/80 backdrop-blur-sm text-indigo-600 px-3 py-1 rounded-full font-medium text-sm shadow-sm border border-indigo-100">
    ...
    <div class="flex-1 p-5 flex flex-col">
      ...
      <div class="bg-gradient-to-br from-indigo-100/30 via-white/50 to-purple-100/30 rounded-2xl p-5 h-64 md:h-72 flex flex-col items-center justify-center mb-5 border border-indigo-100/50 relative overflow-hidden shadow-inner backdrop-blur-sm outfit-preview-area"
        ...
        <div class="absolute top-1/4 left-1/4 w-4 h-4 rounded-full bg-indigo-300/30 animate-ping"></div>
        <div class="absolute bottom-1/3 right-1/3 w-3 h-3 rounded-full bg-purple-300/30 animate-ping animation-delay-700"></div>
        ...
        <div v-else class="w-full h-full flex flex-wrap justify-center items-center gap-3 md:gap-4 relative z-10">
          <div v-for="(item, idx) in selectedClothes.slice(0, 8)" :key="idx"
               ...
               :style="{ transform: `translateY(${(idx % 3 - 1) * 10}px) rotate(${(idx % 2 === 0 ? -1 : 1) * (idx % 3)}deg)`, '--index': idx }"
               :data-index="idx"
      ...
      <!-- 搭配信息表单 -->
      <div class="bg-gradient-to-br from-indigo-50/50 via-white to-purple-50/50 rounded-2xl p-5 mb-5 border border-indigo-100 shadow-sm backdrop-blur-sm">
        ...
        <div class="space-y-4">
          <div>
            <label class="block text-xs font-medium text-indigo-700 mb-1.5">搭配名称 <span class="text-red-500 font-bold ml-1">*</span></label>
            <div class="relative">
              <input
                :value="outfitName"
      ...
      <div class="flex-1 flex flex-col">
        <div class="mb-3 flex justify-between items-center">
          <span class="text-sm font-semibold text-indigo-800">衣物清单 ({{ selectedClothes.length }})</span>
          <button v-if="selectedClothes.length > 0" class="text-xs text-indigo-600 hover:text-indigo-800 transition-colors flex items-center" @click="$emit('reset-clothes')">
            <font-awesome-icon :icon="['fas', 'trash-alt']" class="mr-1" />
            清空全部
          </button>
        </div>

        <div class="h-40 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-indigo-200 scrollbar-track-transparent space-y-2.5">
          <div v-if="selectedClothes.length === 0" class="text-center text-indigo-400 py-6 border-2 border-dashed border-indigo-200 rounded-xl bg-indigo-50/50 backdrop-blur-sm">
          ...
          <div v-else v-for="(item, idx) in selectedClothes" :key="idx"
            ...
            <div class="relative">
              <div class="absolute -inset-1 bg-gradient-to-r from-indigo-200/20 to-purple-200/20 rounded-lg blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
              <img :src="item.img" :alt="item.name" class="w-12 h-12 rounded-lg object-cover shadow-sm relative z-10" loading="lazy" />
              <div class="absolute -top-1.5 -right-1.5 w-6 h-6 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 flex items-center justify-center shadow-sm z-20">
            ...
            <button class="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-400 hover:bg-red-500 hover:text-white transition-all duration-300 opacity-0 group-hover:opacity-100"
        ...
        <div class="mt-5 flex gap-3">
          <button class="flex-1 bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white font-medium py-3.5 rounded-xl transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] shadow-lg hover:shadow-xl flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none relative overflow-hidden group/button"
            ...
            <div class="absolute inset-0 bg-gradient-to-r from-indigo-500/0 via-white/20 to-indigo-500/0 transform -translate-x-full group-hover/button:translate-x-full transition-transform duration-700"></div>
