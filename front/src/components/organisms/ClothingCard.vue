<template>
  ...
  >
    <!-- 操作按钮组（提取公共样式类，减少重复） -->
    <div
      class="card-actions absolute top-2 right-2 z-20 flex flex-col space-y-2 opacity-0 transition-all duration-300 translate-x-2 group-hover:translate-x-0 group-hover:opacity-100"
    >
      <!-- 收藏按钮 -->
      <button
    ...
    <div class="p-3 bg-white border-t border-gray-100">
      <h3 class="font-semibold text-gray-900 truncate text-sm mb-1.5">
        {{ item.name || '未命名衣物' }}
      </h3>
      ...
      <div class="flex flex-wrap gap-1 mt-1.5 max-h-12 overflow-hidden">
        ...
        <span v-if="item.color" class="clothing-tag color-tag">
          {{ getEnumLabel('colors', item.color) }}
        </span>
        <span v-if="item.brand" class="clothing-tag category-tag">
          {{ item.brand }}
        </span>
      </div>
    </div>
