      ...
      >
        ...
        <div class="flex-1 overflow-y-auto p-6 bg-white">
          ...
          <template v-else>
            <!-- 图片上传区域 -->
            <div class="mb-6">
              <ImageUpload v-model:image="form.mainImageUrl" :read-only="props.readOnly" />
            </div>

            <!-- 基本信息 -->
            <div class="mb-6">
              <BasicInfoForm v-model="form" :read-only="props.readOnly" :categories="categories" />
            </div>

            <!-- 季节选择 -->
            <div class="mb-6">
              <SeasonSelector v-model:seasons="form.seasons" :read-only="props.readOnly" />
            </div>

            <!-- 错误提示（优化文案和样式） -->
            <div
              v-if="formError"
              class="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg text-red-600 text-sm flex items-center"
            >
  ...
  const saveItem = async () => {
    ...
    const validateImageUrl = url => {
      ...
      return urlRegex.test(url)
        ? url
        : 'https://via.placeholder.com/300x400/6366f1/ffffff?text=Image';
    };
    ...
    try {
      ...
      // 3. 构造提交数据（精简，避免冗余）
      // 确保季节是数字ID数组
      const seasonIds = Array.isArray(form.value.seasons)
        ? form.value.seasons.map(seasonId => Number(seasonId)).filter(id => !isNaN(id))
        : [];
