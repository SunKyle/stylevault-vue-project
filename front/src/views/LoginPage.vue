  ...
  >
    <div class="flex items-center">
      ...
      <div class="ml-3 flex-1">
        <h3 class="text-base font-semibold leading-6" :class="showSuccess ? 'text-green-800' : 'text-red-800'">
          {{ showRegister ? (showSuccess ? '注册成功' : '注册失败') : (showSuccess ? '登录成功' : '登录失败') }}
        </h3>
        <div class="mt-1.5 text-sm leading-5 whitespace-pre-line" :class="showSuccess ? 'text-green-700' : 'text-red-700'">
        ...
        <button
          ...
          :class="showSuccess ? 'text-green-600 hover:text-green-800 focus:ring-green-400' : 'text-red-600 hover:text-red-800 focus:ring-red-400'"
          @click="closeToast"
        >
          关闭
        </button>
  ...
  if (errorData?.error?.details) {
    ...
    if (Array.isArray(details)) {
      ...
      details.forEach(detail => {
        ...
        if (
          detail.field &&
          Object.prototype.hasOwnProperty.call(errorStore, detail.field)
        ) {
